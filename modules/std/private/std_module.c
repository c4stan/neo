...
void std_module_release ( void* api ) {
    ...
    std_assert_m ( module->handle != NULL );
    char module_entrypoint_name[std_module_name_max_len_m];
...
#ifdef std_platform_win32_m
    std_module_unloader_f unloader = ( std_module_unloader_f ) GetProcAddress ( module->handle, module_entrypoint_name );
...
#else
    std_module_unloader_f unloader = ( std_module_unloader_f ) dlsym ( module->handle, module_entrypoint_name );
...
#define std_neo_path_m "../neo/main.py"
    ...
    char args_buffer[std_process_args_max_len_m];
    {
        ...
        std_str_append ( args_buffer, &array, std_neo_path_m );
        ...
        argv[argc++] = &args_buffer[array.count];
        std_str_append ( args_buffer, &array, "-r" );
    }
    ...
    std_log_info_m ( "Running neo build command for workspace " std_fmt_str_m "...\n", solution_name );
...
void std_module_reload ( const char* solution_name ) {
    ...
    if ( updated_modules_count > 0 ) {
        ...
        for ( int32_t i = 0; i < updated_modules_count; ++i ) {
            ...
            if ( !unload_result ) {
                std_log_error_m ( "Module " std_fmt_str_m " failed to release its old handle after a reload - FreeLibrary failed" );
            }
