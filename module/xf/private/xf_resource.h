...
typedef struct {
    xf_buffer_execution_state_t execution;
    xg_cmd_queue_e prev_queue;
    bool queue_transition;
} xf_buffer_state_t;

#define xf_buffer_state_m( ... ) ( xf_buffer_state_t ) { \
    .execution = xf_buffer_execution_state_m(), \
    .prev_queue = xg_cmd_queue_invalid_m, \
    .queue_transition = false, \
    __VA_ARGS__ \
}
...
typedef struct {
    ...
    bool is_external; // assume all physical textures referenced by this must match
} xf_multi_texture_t;
...
typedef struct {
    ...
    bool is_external; // assume all physical buffers referenced by this must match
} xf_multi_buffer_t;
...
} xf_physical_buffer_params_t;

#define xf_physical_buffer_params_m( ... ) ( xf_physical_buffer_params_t ) { \
    .handle = xg_null_handle_m, \
    __VA_ARGS__ \
}
...
#define xf_physical_buffer_m( ... ) ( xf_physical_buffer_t ) { \
    .handle = xg_null_handle_m, \
    .state = xf_buffer_state_m(), \
    __VA_ARGS__ \
}
...
typedef struct {
    ...
    xf_physical_buffer_t* physical_buffers_array;
    xf_physical_buffer_t* physical_buffers_freelist;
    uint64_t* physical_buffers_bitset;
} xf_resource_state_t;
...
/*
    ...
    The handle returned when creating one is a plain texture handle, which will point ot the 0th backing texture. Handles for
    other backing textures can be retrieved using xf_resource_multi_texture_get_texture. When the multi texture is advanced, all
    references to backing textures will get shifted forward by one.
...
bool xf_resource_texture_is_external ( xf_texture_h texture );
...
xf_physical_buffer_t* xf_resource_buffer_get_physical_buffer ( xf_buffer_h buffer );
void xf_resource_buffer_unmap ( xf_buffer_h buffer );
bool xf_resource_buffer_is_multi ( xf_buffer_h buffer );
bool xf_resource_buffer_is_external ( xf_buffer_h buffer );
