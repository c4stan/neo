...
xf_node_h xf_graph_node_create ( xf_graph_h graph_handle, const xf_node_params_t* params ) {
    ...
    node->renderpass_params.render_textures_desc = xg_render_textures_desc_m();
...
static void xf_graph_add_texture_dependency ( xf_graph_h graph_handle, xf_node_h node_handle, xf_graph_texture_h texture_handle, xg_texture_view_t view, uint32_t resource_idx ) {
    ...
    uint32_t array_count = view.array_count == xg_texture_whole_array_m ? info.array_layers - view.array_base : view.array_count;
    for ( uint32_t array_it = 0; array_it < array_count; ++array_it ) {
        for ( uint32_t mip_it = 0; mip_it < mip_count; ++mip_it ) {
            ...
            uint32_t subres_idx = array_id * info.mip_levels + mip_id;
            std_assert_m ( subres_idx < texture->dependencies.subresource_count );
            xf_graph_subresource_dependencies_t* deps = &texture->dependencies.subresources[subres_idx];
...
static void xf_graph_accumulate_physical_resources_dependencies ( xf_graph_h graph_handle ) {
    ...
    for ( uint32_t node_it = 0; node_it < graph->nodes_count; ++node_it ) {
        ...
        for ( uint32_t tex_it = 0; tex_it < node->textures_count; ++tex_it ) {
            ...
            for ( uint32_t subres_it = 0; subres_it < graph_texture->dependencies.subresource_count; ++subres_it ) {
                xf_graph_subresource_dependencies_t* deps = &graph_texture->dependencies.subresources[subres_it];
                for ( uint32_t access_it = 0; access_it < deps->count; ++access_it ) {
                    ...
                    if ( access->node == node_handle ) {
                        xf_graph_subresource_dependencies_t* acc_deps = &graph_physical_texture->dependencies.subresources[subres_it];
...
static xg_memory_access_bit_e xf_graph_memory_access_from_resource_access ( xf_resource_access_e access ) {
    ...
    case xf_resource_access_indirect_command_m:
        return xg_memory_access_bit_indirect_command_read_m;
    default:
...
static bool xf_graph_queue_supports_stage ( xg_cmd_queue_e queue, xg_pipeline_stage_bit_e stage ) {
    ...
    xg_pipeline_stage_bit_e graphics_stages = 
        xg_pipeline_stage_bit_indirect_command_m |
...
static void xf_graph_add_queue_ownership_transitions ( xf_graph_h graph_handle, xf_node_h node_handle, uint32_t node_resource_idx, xf_texture_execution_state_t* state ) {
    ...
    for ( uint32_t array_it = 0; array_it < array_count; ++array_it ) {
        for ( uint32_t mip_it = 0; mip_it < mip_count; ++mip_it ) {
            ...
            uint32_t array_id = view.array_base + array_it;
            uint32_t subres_idx = array_id * info.mip_levels + mip_id;
            xf_resource_access_t* access = xf_graph_get_subresource_node_access ( &graph_physical_texture->dependencies.subresources[subres_idx], node_handle );

            // Look at the previous access, if any
            if ( access > graph_physical_texture->dependencies.subresources[subres_idx].array ) {
...
static xg_buffer_usage_bit_e xf_graph_buffer_usage_from_access ( xf_resource_access_e access ) {
    ...
    case xf_resource_access_indirect_command_m:
        return xg_buffer_usage_bit_indirect_command_m;
    default:
...
static xg_buffer_params_t xf_graph_buffer_params ( xg_device_h device, xf_buffer_h buffer_handle ) {
    ...
    xg_buffer_params_t params = xg_buffer_params_m (
        ...
        .memory_type = xg_memory_type_gpu_only_m, // TODO readback ?
        .device = device,
...
static xf_graph_resource_dependencies_t xf_graph_alloc_physical_texture_resource_dependencies ( xf_graph_h graph_handle, xf_physical_texture_h physical_texture_handle, xg_texture_view_t view ) {
    ...
    uint32_t array_layers = view.array_count == xg_texture_whole_array_m ? physical_texture->info.array_layers : view.array_count;
    uint32_t subresource_count = mip_count * array_layers;
    deps.subresources = std_stack_alloc_array_m ( &graph->physical_resource_dependencies_allocator, xf_graph_subresource_dependencies_t, subresource_count );
    std_mem_zero_array_m ( deps.subresources, subresource_count );
    deps.subresource_count = subresource_count;
    return deps;
}
...
static void xf_graph_alloc_texture_resource_dependencies ( xf_graph_h graph_handle, xf_graph_texture_h graph_texture_handle ) {
    ...
    uint32_t subresource_count = info.mip_levels * info.array_layers;
    deps.subresources = std_stack_alloc_array_m ( &graph->resource_dependencies_allocator, xf_graph_subresource_dependencies_t, subresource_count );
    std_mem_zero_array_m ( deps.subresources, subresource_count );
    deps.subresource_count = subresource_count;
...
static void xf_graph_scan_buffers ( xf_graph_h graph_handle ) {
    ...
    for ( size_t i = 0; i < graph->nodes_count; ++i ) {
        ...
        if ( node->enabled ) {
            ...
            if ( params->indirect_command_read != xf_null_handle_m ) {
                xf_buffer_h buffer_handle = params->indirect_command_read;
                uint64_t graph_buffer_idx = graph_buffers_array.count;
                if ( std_hash_map_try_insert ( &graph_buffer_idx, &graph_buffers_map, std_hash_64_m ( buffer_handle ), graph_buffer_idx ) ) {
                    std_array_push_m ( &graph_buffers_array, xf_graph_buffer_m ( 
                        .handle = buffer_handle,
                ...
                std_array_push_m ( &node_resources_array, xf_node_resource_m ( 
                    .type = xf_node_resource_buffer_m, 
                    .stage = xg_pipeline_stage_bit_indirect_command_m, 
                    .access = xf_resource_access_indirect_command_m, 
                    .buffer.graph_handle = graph_buffer_idx ) );
                std_array_push_m ( &node_buffers_array, node_resource_idx );
            }
        } else {
...
struct xf_graph_transient_texture_t {
    xf_graph_transient_texture_t* next; // intrusive list of transient textures that alias into the same commited texture
...
typedef struct {
    ...
    xg_memory_requirement_t req;
    xg_texture_params_t params;
    xf_graph_transient_texture_t* transient_textures_list; // list of transient textures that alias into this committed texture
...
static void xf_graph_build_textures ( xf_graph_h graph_handle, xg_i* xg, xg_cmd_buffer_h cmd_buffer, xg_resource_cmd_buffer_h resource_cmd_buffer ) {
    ...
    for ( uint32_t i = 0; i < graph->textures_count; ++i ) {
        ...
        if ( !texture->params.allow_aliasing ) {
    ...
    //       - gather all resources from the list that lifespan time overlap with r
    //       - compute the list of memory ranges that those resources occupy inside the heap
    //       - find a free space outside (in between) those ranges, if it exists
    ...
    for ( uint32_t i = 0; i < committed_textures_count; ++i ) {
        ...
        // Merge contiguous or overlapping memory ranges (can happen when two heap textures don't temporally overlap each other but both overlap the new heap texture candidate)
        ...
        if ( merged_ranges_count == 0 ) {                       // Special case: empty heap
            ...
            if ( aligned_end + req.size <= heap.size ) {        // Special case: heap end
...
static uint64_t xf_graph_clear_owned_textures ( xf_graph_h graph_handle, xg_i* xg, xg_cmd_buffer_h cmd_buffer, uint64_t key ) {
    ...
    for ( uint32_t i = 0; i < graph->owned_textures_count; ++i ) {
        xf_texture_h texture_handle = graph->owned_textures_array[i];
        xf_texture_t* texture = xf_resource_texture_get ( texture_handle );
...
static void xf_graph_build_buffers ( xf_graph_h graph_handle, xg_i* xg, xg_cmd_buffer_h cmd_buffer, xg_resource_cmd_buffer_h resource_cmd_buffer ) {
    ...
    for ( uint32_t i = 0; i < graph->buffers_count; ++i ) {
        ...
        xg_buffer_h xg_buffer = xg->cmd_create_buffer ( resource_cmd_buffer, &params, &buffer->init );
        xg_buffer_info_t info;
        xg->get_buffer_info ( &info, xg_buffer );
        xf_physical_buffer_h physical_buffer_handle = xf_resource_physical_buffer_create ( &xf_physical_buffer_params_m (
            .is_external = false,
            .handle = xg_buffer,
            .info = info,
        ) );
    ...
    for ( uint32_t i = 0; i < graph->multi_buffers_count; ++i ) {
        ...
        for ( uint32_t j = 0; j < multi_buffer->params.multi_buffer_count; ++j ) {
            ...
            xf_buffer_t* buffer = xf_resource_buffer_get ( buffer_handle );
            if ( buffer->physical_buffer_handle == xg_null_handle_m ) {
                xg_buffer_params_t params = xf_graph_buffer_params ( graph->params.device, buffer_handle );
                xg_buffer_h xg_handle = xg->cmd_create_buffer ( resource_cmd_buffer, &params, NULL );
                xg_buffer_info_t info;
                xg->get_buffer_info ( &info, xg_handle );
                xf_physical_buffer_h physical_buffer_handle = xf_resource_physical_buffer_create ( &xf_physical_buffer_params_m (
                    .handle = xg_handle,
                    .info = info,
                ) );
...
static void xf_graph_build_renderpasses ( xf_graph_t* graph, xg_i* xg, xg_resource_cmd_buffer_h resource_cmd_buffer ) {
    for ( uint32_t node_it = 0; node_it < graph->nodes_count; ++node_it ) {
        ...
        xg_render_textures_desc_t render_textures_desc = xg_render_textures_desc_m();
        ...
        for ( size_t resource_it = 0; resource_it < node->params.resources.render_targets_count; ++resource_it ) {
            ...
            render_textures_desc.render_targets[i].usage = physical_texture->info.allowed_usage;
            render_textures_desc.render_targets[i].flags = 0;
            if ( physical_texture->info.flags & xg_texture_flag_bit_cubemap_m ) {
                render_textures_desc.render_targets[i].flags |= xg_texture_create_flag_bit_cubemap_e;
            }
        }
        ...
        // check node depth stencil
        {
            ...
            if ( depth_stencil != xf_null_handle_m ) {
                ...
                render_textures_desc.depth_stencil.usage = physical_texture->info.allowed_usage;
                render_textures_desc.depth_stencil.flags = 0;
            }
        }
        ...
        if ( render_textures_layout.render_targets_count > 0 || render_textures_layout.depth_stencil_enabled ) {
            ...
            if ( !need_update && render_textures_layout.depth_stencil_enabled && ( 
                ...
                || render_textures_desc.depth_stencil.usage != node->renderpass_params.render_textures_desc.depth_stencil.usage
            ) ) {
                need_update = true;
            }
            ...
            if ( !need_update ) {
                for ( uint32_t i = 0; i < render_textures_layout.render_targets_count; ++i ) {
                    if ( 
                        ...
                        || render_textures_desc.render_targets[i].usage != node->renderpass_params.render_textures_desc.render_targets[i].usage
                        || render_textures_desc.render_targets[i].flags != node->renderpass_params.render_textures_desc.render_targets[i].flags
                    ) {
                        need_update = true;
                        break;
                    }
        ...
        if ( need_update ) {
            ...
            xg_renderpass_params_t params = xg_renderpass_params_m (
                ...
                .render_textures_desc = render_textures_desc,
            ...
            node->renderpass_params.render_textures_desc = render_textures_desc;
...
// Can be used to reverse compile and build effects and go back to the original finalized graph state
void xf_graph_clear ( xf_graph_h graph_handle, xg_workload_h workload ) {
    xf_graph_t* graph = &xf_graph_state->graphs_array[graph_handle];
    xg_i* xg = std_module_get_m ( xg_module_name_m );
...
//
void xf_graph_compile ( xf_graph_h graph_handle ) {
    xf_graph_t* graph = &xf_graph_state->graphs_array[graph_handle];
    std_assert_m ( !graph->is_compiled );
...
typedef struct {
    ...
    const xf_node_params_t* params;
} xf_graph_compute_indirect_pass_routine_args_t;

void xf_graph_compute_indirect_pass_routine ( const xf_node_execute_args_t* node_args, void* user_args ) {
    std_auto_m pass_args = ( const xf_graph_compute_indirect_pass_routine_args_t* ) user_args;
    ...
    xg->cmd_compute_indirect ( cmd_buffer, key, &xg_cmd_compute_indirect_params_m ( 
        .pipeline = compute_pipeline,
        .bindings = { xg_null_handle_m, xg_null_handle_m, xg_null_handle_m, group },
        .indirect_buffer = node_args->io->indirect_command_read,
        .indirect_offset = pass_args->params->pass.compute_indirect.indirect_offset,
    ) );
}
    ...
    for ( uint64_t i = 0; i < graph->multi_textures_count; ++i ) {
        ...
        if ( swapchain != xg_null_handle_m ) {
            ...
            if ( !info.acquired ) {
                ...
                if ( acquire.resize ) {
                    xf_resource_multi_texture_swapchain_resize ( multi_texture_handle );
...
#define xf_graph_max_barriers_per_pass_m 128
    ...
    uint32_t wait_events_count = 0;
    {
        ...
        for ( uint32_t i = 0; i < xg_cmd_queue_count_m; ++i ) {
            std_assert_m ( texture_transitions_counts[i] < xf_graph_max_barriers_per_pass_m );
        }
    }
    ...
    for ( size_t node_it = 0; node_it < graph->nodes_count; ++node_it ) {
        ...
        if ( !node->enabled ) {
            ...
            for ( uint32_t i = 0; i < params->resources.storage_buffer_writes_count; ++i ) {
                if ( params->passthrough.storage_buffer_writes[i].mode == xf_passthrough_mode_clear_m ) {
                    ...
                    xf_physical_buffer_t* physical_buffer = xf_resource_buffer_get_physical_buffer ( buffer_write->buffer );
                    xg->cmd_clear_buffer ( cmd_buffer, sort_key, physical_buffer->handle, params->passthrough.storage_buffer_writes[i].clear );
                    ...
                    xf_physical_buffer_t* target_buffer = xf_resource_buffer_get_physical_buffer ( target->buffer );
                    xf_physical_buffer_t* source_buffer = xf_resource_buffer_get_physical_buffer ( source );
                    xg_buffer_copy_params_t copy_params = {
                        .source = source_buffer->handle,
                        .destination = target_buffer->handle,
                    };
            ...
            for ( size_t i = 0; i < node->params.resources.uniform_buffers_count; ++i, ++resource_it ) {
                ...
                xf_graph_buffer_t* graph_buffer = &graph->buffers_array[resource->buffer.graph_handle];
                xf_physical_buffer_t* physical_buffer = xf_resource_buffer_get_physical_buffer ( graph_buffer->handle );
                std_assert_m ( physical_buffer->handle != xg_null_handle_m );
                io.uniform_buffers[i] = physical_buffer->handle;
            ...
            for ( size_t i = 0; i < node->params.resources.storage_buffer_reads_count; ++i, ++resource_it ) {
                ...
                xf_physical_buffer_t* physical_buffer = xf_resource_buffer_get_physical_buffer ( graph_buffer->handle );
                std_assert_m ( physical_buffer->handle != xg_null_handle_m );
                io.storage_buffer_reads[i] = physical_buffer->handle;
            ...
            for ( size_t i = 0; i < node->params.resources.storage_buffer_writes_count; ++i, ++resource_it ) {
                ...
                xf_physical_buffer_t* physical_buffer = xf_resource_buffer_get_physical_buffer ( graph_buffer->handle );
                std_assert_m ( physical_buffer->handle != xg_null_handle_m );
                io.storage_buffer_writes[i] = physical_buffer->handle;
            ...
            for ( size_t i = 0; i < node->params.resources.copy_buffer_reads_count; ++i, ++resource_it ) {
                ...
                xf_physical_buffer_t* physical_buffer = xf_resource_buffer_get_physical_buffer ( graph_buffer->handle );
                std_assert_m ( physical_buffer->handle != xg_null_handle_m );
                io.copy_buffer_reads[i] = physical_buffer->handle;
            ...
            for ( size_t i = 0; i < node->params.resources.copy_buffer_writes_count; ++i, ++resource_it ) {
                ...
                xf_physical_buffer_t* physical_buffer = xf_resource_buffer_get_physical_buffer ( graph_buffer->handle );
                std_assert_m ( physical_buffer->handle != xg_null_handle_m );
                io.copy_buffer_writes[i] = physical_buffer->handle;
            ...
            if ( node->params.resources.indirect_command_read != xf_null_handle_m ) {
                xf_node_resource_t* resource = &node->resources_array[resource_it];
                std_assert_m ( resource->type == xf_node_resource_buffer_m );
                std_assert_m ( resource->access == xf_resource_access_indirect_command_m );
                xf_graph_buffer_t* graph_buffer = &graph->buffers_array[resource->buffer.graph_handle];
                xf_physical_buffer_t* physical_buffer = xf_resource_buffer_get_physical_buffer ( graph_buffer->handle );
                std_assert_m ( physical_buffer->handle != xg_null_handle_m );
                io.indirect_command_read = physical_buffer->handle;
                ++resource_it;
                ...
                io.indirect_command_read = xg_null_handle_m;
            }
        }
        ...
        if ( node->enabled ) {
            ...
            if ( node->params.type == xf_node_type_custom_pass_m ) {
                ...
                std_buffer_t uniform_data = node->params.pass.compute_indirect.uniform_data;

                if ( node->user_alloc ) {
                    uniform_data.base = node->user_alloc;
                }

                std_assert_m ( node->params.pass.compute_indirect.pipeline != xs_null_handle_m );

                xf_graph_compute_indirect_pass_routine_args_t user_args = {
                    .xg = xg,
                    .xs = xs,
                    .pipeline = node->params.pass.compute_indirect.pipeline,
                    .indirect_offset = node->params.pass.compute_indirect.indirect_offset,
                    .uniform_data = uniform_data,
                    .params = &node->params,
                };

                xf_graph_compute_indirect_pass_routine ( &node_args, &user_args );
            } else {
                std_assert_m ( false );
            }
...
// TODO this doesn't work on uncompiled graphs
xf_texture_h xf_graph_get_texture_by_name ( xf_graph_h graph_handle, const char* name ) {
    xf_graph_t* graph = &xf_graph_state->graphs_array[graph_handle];
    for ( uint32_t i = 0; i < graph->textures_count; ++i ) {
        xf_graph_texture_t* graph_texture = &graph->textures_array[i];
        xf_texture_h texture_handle = graph_texture->handle;
        xf_texture_t* texture = xf_resource_texture_get ( texture_handle );
        if ( std_str_cmp ( texture->params.debug_name, name ) == 0 ) {
