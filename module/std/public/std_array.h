...
#define std_array_decl_m( type ) struct std_pp_eval_concat_m ( std_array_, type ) { type* data; uint64_t count; uint64_t capacity; }

#define std_array_m( type, ... ) ( std_array_decl_m ( type ) ) { \
    .data = NULL, \
    .count = 0, \
    .capacity = 0, \
    __VA_ARGS__ \
