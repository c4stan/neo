...
void se_query_resolve ( void ) {
    ...
    for ( size_t query_it = 0; query_it < se_query_state.pending_query_count; ++query_it ) {
        ...
        for ( size_t entity_it = 0; entity_it < entity_count; ++entity_it ) {
            ...
            for ( size_t block_it = 0; block_it < se_component_flags_block_count_m; ++block_it ) {
                uint64_t entity_block = flags[entity_it][block_it];
                uint64_t request_block = pending_query->params.request_component_flags[block_it];
