...
/*
    ...
    The below is for dynamiucally creating layouts, which makes no sense. Layouts are obtained from the pipeline and only sets are dynamically allocated.

    --

    https://themaister.net/blog/2019/04/20/a-tour-of-granites-vulkan-backend-part-3/
        currently using the 'one large pool' design, aka use one VkDescriptorPool per in-flight frame (per thread).
        all descriptor sets for the current frame are allocated in a pool, then the pool gets reset and queued for reuse when the frame is over.
        When calling vkAllocateDescriptorSets an array of VkDescriptorSetLayout must be passed in. One set per VkDescriptorSetLayout will be allocated.
        When calling vkCreateDescriptorSetLayout an array of VkDescriptorSetLayoutCreateInfo must be passed in. One layout per VkDescriptorSetLayoutCreateInfo will be allocated.
        For VkDescriptorSetLayoutCreateInfo an array of VkDescriptorSetLayoutBinding has to be provided.
            VkDescriptorSetLayoutBinding
                u32 shader_register
                VkDescriptorType type
                VkShaderStageFlags shader_stages

        All this considered, a decent way to encode bindings in a xg_cmd_buffer_t could be to simply pass an array of <register, type, stage, resource> in the buffer.
        At parsing time the array gets hashed to look for a matching layout already existing, and if none is found a layout gets created. Then the set gets allocated and the binding gets recorded on the cmd buffer.
    ...
    - cannot share a VkDescriptorPool between multiple threads

    This subsystem needs
        - a pool of VkDescriptorPool sets, one per frame per thread. The set is basically a dynamic array that creates a new VkDescriptorPool if the previous runs out. Call vkAllocateDescriptorSets for each set we need. Put on wait while frame is in flight and recycle when frame is done by calling vkResetDescriptorPool.
        - a pool of vkVkCommandPool set, one per frame (per thread). The set is basically a dynamic array that creates a new VkDescriptorPool if the previous runs out. Call vkAllocateCommandBuffers at setup to pre-initialize a bunch of buffers and lazy allocate more when necessary.
        - a pool of cmd buffers and cmd pool (cmd allocators)

    matrix N_ALLOCATORS_PER_FRAME x N_FRAMES ( x N_LAYOUTS )

    >>>> Implementation for now follows the simple "one big allocator per frame (per thread)" design <<<<
    recycling happens through an array lookup and delete. workload_h is stored for each submit. When a workload is finished a function is expected to be called to notify this submodule.
        TODO this can probably be done in a better way
*/

/*
    What needs to be parallel?
        device activate/deactivate
            need to be safe against multi threaded calling
        workload begin/end
            need to be safe against multi threaded calling

        command recording
            just use one separate cmd buffer per recording thread
        sorting
            the sorting procedure could be parallelized
        chunking
            possibly, needs more investigation
        translation
            once the workload has been separated in chunks, each chunk can be translated independently in parallel
*/
...
typedef struct {
    std_hash_map_t map;
    std_mutex_t mutex;
    uint64_t hashes[xg_vk_cmd_buffer_max_framebuffers_per_translate_m * 2];
    VkFramebuffer framebuffers[xg_vk_cmd_buffer_max_framebuffers_per_translate_m * 2];
} xg_vk_framebuffer_cache_t;
*/

// ---

/*
    sort input:  array of xg_cmd_buffer items
    sort output: single buffer containing all sorted cmd headers coming from the xg_cmd_buffers
...
} xg_vk_cmd_buffer_translation_context_t;

typedef struct {
    xg_vk_cmd_buffer_translation_context_t* translation_contexts[xg_vk_cmd_buffer_max_translates_per_submit_m];
    xg_vk_fence_t* fence;
    xg_vk_device_t* device;
    xg_workload_h workload;
} xg_vk_cmd_buffer_submission_context_t;
*/

/*
// Should be 64 or lower since it's using a u64 bitset to track usage
#define xg_resource_bindings_max_registers_per_type_per_stage_m   64

typedef struct {
    uint64_t bitset;
    xg_resource_h resources[xg_resource_bindings_max_registers_per_type_per_stage_m];
} xg_vk_cmd_buffer_submit_context_register_usage_t;

typedef struct {
    bool dirty;
    xg_vk_cmd_buffer_submit_context_register_usage_t register_usage[xg_shading_stage_count_m][xg_resource_binding_count_m];
} xg_vk_cmd_buffer_submit_context_bindings_set_t;
*/

/*
    device_contexts[device_handle]
        submit_contexts<pool>
            workload_handle

    can lookup the device context by simply accessing the array with the handle
    looking up the submit context is more complex
        could have a workload_handle->pool index/ptr map per device_context

*/

static xg_vk_cmd_buffer_state_t* xg_vk_cmd_buffer_state;

// API
void xg_vk_cmd_buffer_load ( xg_vk_cmd_buffer_state_t* state ) {
    xg_vk_cmd_buffer_state = state;

    std_mem_zero_m ( &xg_vk_cmd_buffer_state->device_contexts );

    std_mutex_init ( &xg_vk_cmd_buffer_state->device_contexts_mutex );
}
...
void xg_vk_cmd_buffer_unload ( void ) {
    ...
    for ( uint32_t i = 0; i < xg_vk_max_active_devices_m; ++i ) {
        ...
        for ( uint32_t i = 0; i < xg_workload_max_queued_workloads_m; ++i ) \
        {
            xg_vk_cmd_buffer_submit_context_t* submit_context = &context->submit_contexts[i];
...
static void xg_vk_cmd_buffer_debug_print_cmd_buffer ( const std_buffer_t* cmd_headers ) {
    ...
    const xg_cmd_header_t* cmd_headers_end = ( xg_cmd_header_t* ) ( cmd_headers->base + cmd_headers->size );

    uint64_t total_cmd_count = ( cmd_headers_end - cmd_header ) / sizeof ( xg_cmd_header_t* );
    uint64_t cmd_count = 0;

    std_log_info_m ( "PRINTING CONTENTS OF CMD BUFFER " std_fmt_ptr_m " (" std_fmt_u64_m " COMMANDS)", cmd_headers->base, total_cmd_count );

#define xg_vk_get_resource_debug_name(_res, _name) \
        if ( _res ) { \
            if ( _res->params.debug_name ) { \
                _name = _res->params.debug_name; \
            } else { \
                _name = "NAMELESS_RESOURCE"; \
            } \
        } else { \
            _name = "INVALID_RESOURCE"; \
        } \

#define xg_vk_get_pipeline_debug_name(_res, _name) \
        if ( _res ) { \
            if ( _res->common.debug_name ) { \
                _name = _res->common.debug_name; \
            } else { \
                _name = "NAMELESS_RESOURCE"; \
            } \
        } else { \
    ...
    for ( const xg_cmd_header_t* header = cmd_header; header < cmd_headers_end; ++header ) {
        switch ( header->type ) {
            case xg_cmd_graphics_streams_bind_m: {
                ...
                std_log_info_m ( "CMD " std_fmt_u64_m " - GRAPHICS_STREAMS_BIND (" std_fmt_u32_m ")", cmd_count++, xg_cmd_graphics_streams_bind_m );

                for ( uint32_t i = 0; i < args->bindings_count; ++i ) {
                    xg_vertex_stream_binding_t* binding = &args->bindings[i];

                    const xg_vk_buffer_t* buffer = xg_vk_buffer_get ( binding->buffer );
                    const char* name;
                    xg_vk_get_resource_debug_name ( buffer, name );

                    std_log_info_m ( std_fmt_tab_m "Binding " std_fmt_u32_m ": Stream " std_fmt_u32_m " Buffer " std_fmt_u64_m "(" std_fmt_str_m ")" " Offset " std_fmt_u32_m,
                        i, binding->stream_id, binding->buffer, name, binding->offset );
                }
            }
            break;
            ...
            case xg_cmd_graphics_pipeline_state_bind_m: {
                ...
                std_log_info_m ( "CMD " std_fmt_u64_m " - GRAPHICS_PIPELINE_STATE_BIND (" std_fmt_u32_m ")", cmd_count++, xg_cmd_graphics_pipeline_state_bind_m );

                const char* name;
                xg_vk_get_pipeline_debug_name ( pipeline, name );

                std_log_info_m ( std_fmt_tab_m "Pipeline " std_fmt_u64_m "(" std_fmt_str_m ")", args->pipeline, name );
            }
            break;
            ...
            case xg_cmd_graphics_pipeline_state_set_viewport_m: {
                ...
                std_log_info_m ( "CMD " std_fmt_u64_m " - SET_VIEWPORT (" std_fmt_u32_m ")", cmd_count++, xg_cmd_graphics_pipeline_state_set_viewport_m );

                std_log_info_m ( std_fmt_tab_m "Viewport x:" std_fmt_u32_m " y:" std_fmt_u32_m " w:" std_fmt_u32_m " h:" std_fmt_u32_m " mind:" std_fmt_f32_dec_m ( 5 ) " maxd:" std_fmt_f32_dec_m ( 5 ),
                    args->viewport.x, args->viewport.y, args->viewport.width, args->viewport.height, args->viewport.min_depth, args->viewport.max_depth );
            }
            break;
            ...
            case xg_cmd_graphics_pipeline_state_set_scissor_m: {
                ...
                std_log_info_m ( "CMD " std_fmt_u64_m " - SET_SCISSOR (" std_fmt_u32_m ")", cmd_count++, xg_cmd_graphics_pipeline_state_set_scissor_m );

                std_log_info_m ( std_fmt_tab_m "Scissor x:" std_fmt_u32_m " y:" std_fmt_u32_m " w:" std_fmt_u32_m " h:" std_fmt_u32_m,
                    args->scissor.x, args->scissor.y, args->scissor.width, args->scissor.height );
            }
            break;
            ...
            case xg_cmd_compute_pipeline_state_bind_m: {
                ...
                std_log_info_m ( "CMD " std_fmt_u64_m " - COMPUTE_PIPELINE_STATE_BIND (" std_fmt_u32_m ")", cmd_count++, xg_cmd_compute_pipeline_state_bind_m );
            ...
            case xg_cmd_pipeline_constant_write_m: {
                ...
                std_log_info_m ( "CMD " std_fmt_u64_m " - PIPELINE_CONSTANT_WRITE (" std_fmt_u32_m ")", cmd_count++, xg_cmd_pipeline_constant_write_m );
            ...
            case xg_cmd_pipeline_resource_group_bind_m: {
                ...
                std_log_info_m ( "CMD " std_fmt_u64_m " - RESOURCE_GROUP_BIND (" std_fmt_u32_m ")", cmd_count++, xg_cmd_pipeline_resource_group_bind_m );
            ...
            case xg_cmd_pipeline_resource_bind_m: {
                ...
                std_log_info_m ( "CMD " std_fmt_u64_m " - PIPELINE_RESOURCE_BIND (" std_fmt_u32_m ")", cmd_count++, xg_cmd_pipeline_resource_bind_m );
                ...
                xg_resource_binding_set_e set = args->set;

                std_log_info_m ( std_fmt_tab_m "Set " std_fmt_u32_m, set );

                for ( uint32_t i = 0; i < buffer_count; ++i ) {
                    xg_buffer_resource_binding_t* binding = &buffers[i];
                    const xg_vk_buffer_t* buffer = xg_vk_buffer_get ( binding->range.handle );
                    const char* name;
                    xg_vk_get_resource_debug_name ( buffer, name );

                    // TODO write out binding->type
                    std_log_info_m ( std_fmt_tab_m "Buffer " std_fmt_u32_m ": " "Register " std_fmt_u32_m " -> " std_fmt_u64_m "(" std_fmt_str_m ")" " offset:" std_fmt_u64_m " size:" std_fmt_u64_m,
                        i, binding->shader_register, binding->range.handle, name, binding->range.offset, binding->range.size );
                }

                for ( uint32_t i = 0; i < texture_count; ++i ) {
                    xg_texture_resource_binding_t* binding = &textures[i];
                    const xg_vk_texture_t* texture = xg_vk_texture_get ( binding->texture );
                    const char* name;
                    xg_vk_get_resource_debug_name ( texture, name );

                    // TODO write out layout and view
                    std_log_info_m ( std_fmt_tab_m "Texture " std_fmt_u32_m ": " "Register " std_fmt_u32_m " -> " std_fmt_u64_m "(" std_fmt_str_m ")",
                        i, binding->shader_register, binding->texture, name );
                }

                for ( uint32_t i = 0; i < sampler_count; ++i ) {
                    xg_sampler_resource_binding_t* binding = &samplers[i];
                    std_log_info_m ( std_fmt_tab_m "Sampler " std_fmt_u32_m ": " "Register " std_fmt_u32_m " -> " std_fmt_u64_m,
                        i, binding->shader_register, binding->sampler );
                    std_unused_m ( samplers );
                }
            }
            break;
            ...
            case xg_cmd_graphics_render_textures_bind_m: {
                ...
                std_log_info_m ( "CMD " std_fmt_u64_m " - RENDER_TARGET_BIND (" std_fmt_u32_m ")", cmd_count++, xg_cmd_graphics_render_textures_bind_m );
            }
            break;
            ...
            case xg_cmd_graphics_streams_submit_m: {
                ...
                std_log_info_m ( "CMD " std_fmt_u64_m " - GRAPHICS_STREAMS_SUBMIT (" std_fmt_u32_m ")", cmd_count++, xg_cmd_graphics_streams_submit_m );
            }
            break;
            ...
            case xg_cmd_graphics_streams_submit_indexed_m: {
                ...
                std_log_info_m ( "CMD " std_fmt_u64_m " - GRAPHICS_STREAMS_SUBMIT_INDEXED (" std_fmt_u32_m ")", cmd_count++, xg_cmd_graphics_streams_submit_indexed_m );
            }
            break;
            ...
            case xg_cmd_graphics_streams_submit_instanced_m: {
                ...
                std_log_info_m ( "CMD " std_fmt_u64_m " - GRAPHICS_STREAMS_SUBMIT_INSTANCED (" std_fmt_u32_m ")", cmd_count++, xg_cmd_graphics_streams_submit_instanced_m );
            }
            break;
            ...
            case xg_cmd_compute_dispatch_m: {
                ...
                std_log_info_m ( "CMD " std_fmt_u64_m " - COMPUTE_DISPATCH (" std_fmt_u32_m ")", cmd_count++, xg_cmd_compute_dispatch_m );
            }
            break;
            ...
            case xg_cmd_copy_buffer_m : {
                ...
                std_log_info_m ( "CMD " std_fmt_u64_m " - COPY_BUFFER (" std_fmt_u32_m ")", cmd_count++, xg_cmd_copy_buffer_m );
            }
            break;
            ...
            case xg_cmd_copy_texture_m : {
                ...
                std_log_info_m ( "CMD " std_fmt_u64_m " - COPY_TEXTURE (" std_fmt_u32_m ")", cmd_count++, xg_cmd_copy_texture_m );
            }
            break;
            ...
            case xg_cmd_copy_texture_to_buffer_m: {
                ...
                std_log_info_m ( "CMD " std_fmt_u64_m " - COPY_TEXTURE_TO_BUFFER (" std_fmt_u32_m ")", cmd_count++, xg_cmd_copy_texture_to_buffer_m );
            }
            break;
            ...
            case xg_cmd_copy_buffer_to_texture_m: {
                ...
                std_log_info_m ( "CMD " std_fmt_u64_m " - COPY_BUFFER_TO_TEXTURE (" std_fmt_u32_m ")", cmd_count++, xg_cmd_copy_buffer_to_texture_m );
            }
            break;
            ...
            case xg_cmd_barrier_set_m: {
                ...
                std_log_info_m ( "CMD " std_fmt_u64_m " - BARRIER_SET (" std_fmt_u32_m ")", cmd_count++, xg_cmd_barrier_set_m );
            }
            break;
            ...
            case xg_cmd_texture_clear_m: {
                ...
                std_log_info_m ( "CMD " std_fmt_u64_m " - TEXTURE_CLEAR (" std_fmt_u32_m ")", cmd_count++, xg_cmd_texture_clear_m );
            }
            break;
            ...
            case xg_cmd_texture_depth_stencil_clear_m: {
                ...
                std_log_info_m ( "CMD " std_fmt_u64_m " - DEPTH_STENCIL_CLEAR (" std_fmt_u32_m ")", cmd_count++, xg_cmd_texture_depth_stencil_clear_m );
            }
            break;
            ...
            case xg_cmd_start_debug_capture_m: {
                ...
                std_log_info_m ( "CMD " std_fmt_u64_m " - START_DEBUG_CAPTURE (" std_fmt_u32_m ")", cmd_count++, xg_cmd_start_debug_capture_m );
            }
            break;

            // -----------------------------------------------------------------------
            case xg_cmd_stop_debug_capture_m: {
                std_log_info_m ( "CMD " std_fmt_u64_m " - STOP_DEBUG_CAPTURE (" std_fmt_u32_m ")", cmd_count++, xg_cmd_stop_debug_capture_m );
            }
            break;
            ...
            case xg_cmd_begin_debug_region_m: {
                ...
                std_log_info_m ( "CMD " std_fmt_u64_m " - BEGIN_DEBUG_REGION (" std_fmt_u32_m ")", cmd_count++, xg_cmd_begin_debug_region_m );

                std_log_info_m ( std_fmt_tab_m "Begin " std_fmt_str_m, args->name );
            }
            break;

            // -----------------------------------------------------------------------
            case xg_cmd_end_debug_region_m: {
                std_log_info_m ( "CMD " std_fmt_u64_m " - END_DEBUG_REGION (" std_fmt_u32_m ")", cmd_count++, xg_cmd_end_debug_region_m );
            }
            break;
            ...
            case xg_cmd_write_timestamp_m: {
                ...
                std_log_info_m ( "CMD " std_fmt_u64_m " - WRITE_TIMESTAMP (" std_fmt_u32_m ")", cmd_count++, xg_cmd_write_timestamp_m );
            }
            break;
#endif
        }
...
void xg_vk_cmd_buffer_activate_device ( xg_device_h device_handle ) {
    ...
    xg_vk_cmd_buffer_device_context_t* context = &xg_vk_cmd_buffer_state->device_contexts[device_idx];
    ...
    for ( size_t i = 0; i <  xg_workload_max_queued_workloads_m; ++i ) {
        ...
        // cmd pool
        {
            ...
            VkResult result = vkCreateCommandPool ( device->vk_handle, &info, NULL, &context->submit_contexts[i].cmd_allocator.vk_cmd_pool );
        ...
        // cmd buffers
        {
            ...
            info.pNext = NULL;
            info.commandPool = context->submit_contexts[i].cmd_allocator.vk_cmd_pool;
            info.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;                               // TODO allocate some primary and some secondary
            info.commandBufferCount = xg_vk_cmd_buffer_cmd_buffers_per_allocator_m;     // TODO preallocate only some and allocate more when we run out?
            VkResult result = vkAllocateCommandBuffers ( device->vk_handle, &info, context->submit_contexts[i].cmd_allocator.vk_cmd_buffers );
            xg_vk_assert_m ( result );
        }
    ...
    //std_atomic_increment_u64 ( &xg_vk_cmd_buffer_state->device_contexts_count );
}
...
/*
    ...
    various other sources and notes:
        ...
        Radix + counting sort:
            on some radix sort iterations is might be worth to do a counting sort pass instead of a radix
            radix sort does a single pass of the items and splits them into different bins
...
typedef struct {
    ...
    //xg_resource_binding_t bindings[xg_resource_binding_set_count_m][xg_pipeline_resource_max_bindings_per_set_m];
    //uint32_t binding_counts[xg_resource_binding_set_count_m];
    //bool dirty_binding_sets;//[xg_resource_binding_set_count_m];
    xg_vk_translation_resource_binding_set_cache_t sets[xg_resource_binding_set_count_m];

    //xg_pipeline_resource_group_h groups[xg_resource_binding_set_count_m];
...
static void xg_vk_translation_state_cache_flush ( xg_vk_tranlsation_state_t* state, const xg_vk_cmd_buffer_submit_context_t* context, xg_device_h device_handle, VkCommandBuffer vk_cmd_buffer ) {
    ...
    if ( state->dirty_pipeline ) {
        ...
        uint32_t first_deactivated_set;
        ...
        for ( uint32_t set = first_deactivated_set; set < xg_resource_binding_set_count_m; ++set ) {
    ...
    for ( uint32_t set_it = 0; set_it < xg_resource_binding_set_count_m; ++set_it ) {
        ...
        //      in this case an old binding has become once again valid because it is now compatible with the active pipeline
        ...
        if ( vk_set == VK_NULL_HANDLE ) {
            ...
            for ( size_t i = 0; i < state->sets[set].bindings_count; ++i ) {
                xg_resource_binding_t* binding = &state->sets[set].bindings[i];
                ...
                //VkBufferView vk_texel_buffer_info;

                switch ( binding->type ) {
                    case xg_resource_binding_buffer_uniform_m:
                        write->descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
                        write->pBufferInfo = &vk_buffer_info[i];
                        vk_buffer_info[i].offset = 0;
                        vk_buffer_info[i].range = VK_WHOLE_SIZE;
                        vk_buffer_info[i].buffer = xg_vk_buffer_get ( binding->resource )->vk_handle;
                        break;

                    case xg_resource_binding_texture_to_sample_m:
                        write->descriptorType = VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE;
                        write->pImageInfo = &vk_image_info[i];
                        vk_image_info[i].sampler = VK_NULL_HANDLE;
                        const xg_vk_texture_t* texture = xg_vk_texture_get ( binding->resource );
                        vk_image_info[i].imageView = texture->vk_view;

                        if ( texture->flags & xg_texture_flag_bit_depth_stencil_texture_m ) {
                            vk_image_info[i].imageLayout = xg_image_layout_to_vk ( xg_texture_layout_depth_stencil_read_m );
                        } else {
                            vk_image_info[i].imageLayout = xg_image_layout_to_vk ( xg_texture_layout_shader_read_m );
                        }
                    ...
                    case xg_resource_binding_sampler_m:
                        write->descriptorType = VK_DESCRIPTOR_TYPE_SAMPLER;
                        write->pImageInfo = &vk_image_info[i];
                        vk_image_info[i].sampler = xg_vk_sampler_get ( binding->resource )->vk_handle;
                        vk_image_info[i].imageView = VK_NULL_HANDLE;
                        break;
...
static void xg_vk_submit_context_translate ( xg_vk_cmd_translate_result_t* result, xg_vk_cmd_buffer_submit_context_t* context, xg_device_h device_handle ) {
    ...
    for ( size_t cmd_header_it = 0; cmd_header_it < context->cmd_headers_count; ++cmd_header_it ) {
        ...
        switch ( header->type ) {
            ...
            case xg_cmd_pipeline_resource_group_bind_m: {
                ...
                const xg_vk_pipeline_resource_group_t* group = xg_vk_pipeline_resource_group_get ( device_handle, args->group );
...
static void xg_vk_cmd_buffer_update_resource_groups ( xg_workload_h workload_handle ) {
    const xg_vk_workload_t* workload = xg_vk_workload_get ( workload_handle );
...
static void xg_vk_cmd_buffer_create_workload_resources ( xg_workload_h workload_handle ) {
    const xg_vk_workload_t* workload = xg_vk_workload_get ( workload_handle );
    std_assert_m ( workload );
...
static void xg_vk_cmd_buffer_destroy_workload_resources ( xg_workload_h workload_handle, xg_resource_cmd_buffer_time_e destroy_time ) {
    const xg_vk_workload_t* workload = xg_vk_workload_get ( workload_handle );
    std_assert_m ( workload );
...
#if std_enabled_m(xg_debug_measure_workload_wait_time_m)
    size_t initial_count = count;
#endif

    while ( count > 0 ) {
        xg_vk_cmd_buffer_submit_context_t* submit_context = &device_context->submit_contexts[std_ring_bot_idx ( &device_context->submit_ring )];
        ...
        xg_vk_cmd_buffer_destroy_workload_resources ( submit_context->workload, xg_resource_cmd_buffer_time_workload_complete_m );
        xg_workload_destroy ( submit_context->workload );
        submit_context->workload = xg_null_handle_m;

        std_ring_pop ( &device_context->submit_ring, 1 );

        count = std_ring_count ( &device_context->submit_ring );
    }
...
void xg_vk_cmd_buffer_submit ( xg_workload_h workload_handle ) {
    ...
    xg_vk_cmd_buffer_device_context_recycle_submission_contexts ( device_context );
    ...
    // test cmd buffers, if empty process resource cmd buffers and exit here
    {
        ...
        if ( total_header_size == 0 ) {
            ...
            xg_vk_cmd_buffer_create_workload_resources ( workload_handle );
            xg_vk_cmd_buffer_update_resource_groups ( workload_handle );
            xg_vk_cmd_buffer_destroy_workload_resources ( workload_handle, xg_resource_cmd_buffer_time_workload_start_m );
            xg_vk_cmd_buffer_destroy_workload_resources ( workload_handle, xg_resource_cmd_buffer_time_workload_complete_m );
    ...
    xg_vk_cmd_buffer_submit_context_t* submit_context = &device_context->submit_contexts[std_ring_top_idx ( &device_context->submit_ring )];
    ...
    if ( translate_result.debug_capture_stop_on_workload_present ) {
        xg_vk_workload_queue_debug_capture_stop_on_present ( workload_handle );
    }
}
