...
static void hz_gen_pass ( const xf_node_execute_args_t* node_args, void* user_args ) {
    ...
    xs_i* xs = std_module_get_m ( xs_module_name_m );
    {
        ...
        xg_buffer_range_t shader_buffer_range = xg->write_workload_uniform ( node_args->workload, &pass_args->draw_data, sizeof ( hz_gen_draw_data_t ) );
...
xf_node_h add_hiz_mip0_gen_pass ( xf_graph_h graph, xf_texture_h hiz, xf_texture_h depth ) {
    ...
    args.pipeline = xs->lookup_pipeline_state ( "copy" );
    args.sampler = xg->get_default_sampler ( graph_info.device, xg_default_sampler_point_clamp_m );
    args.width = hiz_info.width;
    args.height = hiz_info.height;

    xf_node_params_t params = xf_default_node_params_m;
    params.render_targets[params.render_targets_count++] = xf_render_target_dependency_m ( hiz, xg_texture_view_m ( .mip_base = 0, .mip_count = 1 ) );
...
xf_node_h add_hiz_submip_gen_pass ( xf_graph_h graph, xf_texture_h hiz, uint32_t mip_level ) {
    ...
    hz_gen_pass_args_t args;
    args.pipeline = xs->lookup_pipeline_state ( "hz_gen_fs" );
    args.sampler = xg->get_default_sampler ( graph_info.device, xg_default_sampler_point_clamp_m );
    args.draw_data.src_resolution_x_f32 = ( float ) ( hiz_info.width / ( 1 << ( mip_level - 1 ) ) );
    args.draw_data.src_resolution_y_f32 = ( float ) ( hiz_info.height / ( 1 << ( mip_level - 1 ) ) );
    args.draw_data.dst_resolution_x_f32 = ( float ) ( hiz_info.width / ( 1 << mip_level ) );
    args.draw_data.dst_resolution_y_f32 = ( float ) ( hiz_info.height / ( 1 << mip_level ) );
    args.width = hiz_info.width / ( 1 << mip_level );
    args.height = hiz_info.height / ( 1 << mip_level );

    xf_node_params_t params = xf_default_node_params_m;
    params.render_targets[params.render_targets_count++] = xf_render_target_dependency_m ( hiz, xg_texture_view_m ( .mip_base = mip_level, .mip_count = 1 ) );
    params.shader_texture_reads[params.shader_texture_reads_count++] = xf_shader_texture_dependency_m ( hiz, xg_texture_view_m ( .mip_base = mip_level - 1, .mip_count = 1 ), xg_shading_stage_fragment_m );
    params.execute_routine = hz_gen_pass;
    params.user_args = std_buffer_m ( &args );
