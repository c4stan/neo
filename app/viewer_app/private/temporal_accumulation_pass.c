...
static void temporal_accumulation_pass ( const xf_node_execute_args_t* node_args, void* user_args ) {
    ...
    xs_i* xs = std_module_get_m ( xs_module_name_m );
    {
...

        {
            ...
            xg_buffer_range_t shader_buffer_range = xg->write_workload_uniform ( node_args->workload, &pass_args->draw_data, sizeof ( temporal_accumulation_draw_data_t ) );
...
xf_node_h add_temporal_accumulation_pass ( xf_graph_h graph, xf_texture_h accumulation, xf_texture_h color, xf_texture_h history, xf_texture_h depth, xf_texture_h prev_depth, const char* debug_name ) {
    ...
    {
        args.pipeline = xs->lookup_pipeline_state ( "ta" );
        args.sampler = xg->get_default_sampler ( graph_info.device, xg_default_sampler_point_clamp_m );
        args.draw_data.resolution_x = ( float ) accumulation_info.width;
        args.draw_data.resolution_y = ( float ) accumulation_info.height;
        args.width = accumulation_info.width;
        args.height = accumulation_info.height;
    }

    xf_node_h temporal_accumulation_node;
    {
        xf_node_params_t params = xf_default_node_params_m;
        params.render_targets[params.render_targets_count++] = xf_render_target_dependency_m ( accumulation, xg_default_texture_view_m );
        ...
        params.shader_texture_reads[params.shader_texture_reads_count++] = xf_sampled_texture_dependency_m ( history, xg_shading_stage_fragment_m );
        ...
        temporal_accumulation_node = xf->create_node ( graph, &params );
    }
